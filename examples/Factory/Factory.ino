...
static int16_t tileview_x;
static int16_t tileview_y;
static lv_obj_t *screen_move_label;
static lv_obj_t *direct_label;
static uint8_t direct_icon;
static bool screen_move_adjusting;
...
static void lv_gui_select_next_direction();
static void lv_gui_set_screen_move_adjusting(bool enabled);
static void lv_gui_load_tileview_position();
static void lv_gui_save_tileview_position();
static void lv_gui_clamp_tileview_position();
static void lv_gui_move_tileview();
...
void boot_button_event_callback(ButtonState state)
{
    ...
    case BTN_CLICK_EVENT:
        if (current_page == PAGE_SCREEN_MOVE && screen_move_adjusting)
        {
            lv_gui_select_next_direction();
        }
        break;
    ...
    case BTN_DOUBLE_CLICK_EVENT:
        ...
        if (current_page == PAGE_SCREEN_MOVE)
        {
            if (screen_move_adjusting)
            {
                lv_gui_set_screen_move_adjusting(false);
                lv_gui_save_tileview_position();
                lv_gui_select_next_item();
            }
            else
            {
                lv_gui_set_screen_move_adjusting(true);
            }
        }
        break;
...
static void lv_tileview_add_img(lv_obj_t *parent)
{
    ...
    lv_timer_t *timer = lv_timer_create([](lv_timer_t *t)
        ...
        lv_anim_start(&a);
        i++;
        i %= (sizeof(str) / sizeof(str[0])); }, 3000, label);
...
static void lv_tileview_add_screen_move(lv_obj_t *parent)
{
    screen_move_label = lv_label_create(parent);
    lv_obj_set_style_text_color(screen_move_label, font_color, LV_PART_MAIN);
    lv_obj_set_style_text_font(screen_move_label, &lv_font_montserrat_12, LV_PART_MAIN);
    lv_label_set_text(screen_move_label, "Screen Move");
    lv_obj_align(screen_move_label, LV_ALIGN_TOP_MID, 0, 0);

    direct_label = lv_label_create(parent);
    lv_obj_set_style_text_color(direct_label, font_color, LV_PART_MAIN);
    lv_obj_set_style_text_font(direct_label, &lv_font_montserrat_20, LV_PART_MAIN);
    lv_label_set_text(direct_label, "UP");
    lv_obj_align(direct_label, LV_ALIGN_CENTER, 0, 0);
}
...
static const char *direct_text[] = {"UP", "DOWN", "LEFT", "RIGHT"};

static void lv_gui_set_screen_move_adjusting(bool enabled)
{
    screen_move_adjusting = enabled;
    if (screen_move_label)
    {
        lv_label_set_text(screen_move_label, enabled ? "Adjusting" : "Screen Move");
        lv_obj_align(screen_move_label, LV_ALIGN_TOP_MID, 0, 0);
    }
}
...
static void lv_gui_load_tileview_position()
{
    if (!preferences.begin("glass_config", true))
    {
        Serial.println("Failed to open preferences for tileview position");
        return;
    }

    tileview_x = preferences.getShort("tileview_x", 0);
    tileview_y = preferences.getShort("tileview_y", 0);
    preferences.end();

    lv_gui_clamp_tileview_position();
    Serial.printf("Loaded tileview offset x:%d y:%d\n", tileview_x, tileview_y);
}

static void lv_gui_save_tileview_position()
{
    lv_gui_clamp_tileview_position();
    if (!preferences.begin("glass_config", false))
    {
        Serial.println("Failed to open preferences for saving tileview position");
        return;
    }

    preferences.putShort("tileview_x", tileview_x);
    preferences.putShort("tileview_y", tileview_y);
    preferences.end();

    Serial.printf("Saved tileview offset x:%d y:%d\n", tileview_x, tileview_y);
}

static void lv_gui_select_next_direction()
{
    direct_icon++;
    direct_icon %= sizeof(direct_text) / sizeof(direct_text[0]);
    if (direct_label)
    {
        lv_label_set_text(direct_label, direct_text[direct_icon]);
        lv_obj_align(direct_label, LV_ALIGN_CENTER, 0, 0);
    }
}
...
static void lv_gui_move_tileview()
{
    ...
    Serial.printf("Tileview offset x:%d y:%d direction:%s\n", tileview_x, tileview_y, direct_text[direct_icon]);
}

static void lv_gui_select_next_item()
{
    uint32_t page_id = lv_obj_get_index(lv_tileview_get_tile_act(tileview));
    uint32_t page_count = lv_obj_get_child_cnt(tileview);
    page_id++;
    page_id %= page_count;
    current_tile = page_id;
...
static void lv_gui_init()
{
    ...
    tileview_y = 0;
    direct_icon = 0;
    screen_move_adjusting = false;
    lv_gui_load_tileview_position();
    ...
    // Create screen move page
    lv_tileview_add_screen_move(t8);
}
